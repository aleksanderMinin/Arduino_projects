/* программа для управления спаркой вентиляторов радиатора
  ...
  при включании запускаются вентиляторы на 4 сек для проверки работы
  включается одна обмотка одного вентилятора при 80
  при 85* включается один на полную
  при 90 включается второй на полную
  2016год
  ячейкb EEPROM[0..8] - точки температуры
  ячейка EEPROM[9] - флаг вкл/выкл вентиляторы в начале пуска
  ячейка EEPROM[10] - ремя работы в первое включение
...
void Talk() {
  if (Serial.available() > 0 || smsbuffer.length() > 0) {   //если есть сообщение
    ...
    if (sms.substring(0, 4) == "help") {
      ...
      Serial.println("manualN - to manual turn On only N number relay");
...
void loop() {
  ...
  temp_toj = analogRead(analogPin); //читаем температуру
  //Serial.print("toj_temp_");
  //Serial.println(temp_toj);   //вывод температуры тож в консоль
  sms = "toj_temp_";
  sms += temp_toj;
  Serial.println(sms);
  ...
  if (temp_toj >= temp[2] && temp_toj < temp[3] && manual == 0) RelayState(1, 0, 0, 0);

  // включает 1 мотор полность
  //if (temp >= 85 && temp < 87 && hardon != 1) {
  if (temp_toj >= temp[4] && temp_toj < temp[5] &&  manual == 0) RelayState(1, 1, 0, 0);

  // включает 1 мотор полность и половину второго
  //if (temp >= 87 && temp < 90 && hardon != 1) {
  if (temp_toj >= temp[6] && temp_toj < temp[7] &&  manual == 0) RelayState(1, 1, 1, 0);
  ...
  //Temp();                   //меряем температуру
  //MCPRecive();              //читаем сообщение по can шине
